    ...
    describe('POST /v1/auth/register', () => {
        ...
        test('should return 201 and successfully register user if request data is ok', async () => {
            const res = await request(app)
                ...
                .send(newUser)
                .expect(httpStatus.status.CREATED)
        ...
        test('should return 400 error if email is invalid', async () => {
            ...
            await request(app)
                .post('/v1/auth/register')
                .send(newUser)
                .expect(httpStatus.status.BAD_REQUEST)
        })
    ...
    describe('POST /v1/auth/login', () => {
        ...
        test('should return 401 error if there are no users with that email', async () => {
            ...
            const res = await request(app)
                .post('/v1/auth/login')
                .send(loginCredentials)
                .expect(httpStatus.status.UNAUTHORIZED)

            expect(res.body).toEqual({
                code: httpStatus.status.UNAUTHORIZED,
    ...
    describe('POST /v1/auth/logout', () => {
        test('should return 204 if refresh token is valid', async () => {
            ...
            await request(app)
                .post('/v1/auth/logout')
                .send({ refreshToken })
                .expect(httpStatus.status.NO_CONTENT)
        ...
        test('should return 404 error if refresh token is not found in the database', async () => {
            ...
            await request(app)
                .post('/v1/auth/logout')
                .send({ refreshToken })
                .expect(httpStatus.status.NOT_FOUND)
        })
